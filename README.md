...
| 课设要求 | 实现情况 |
|---------|---------|
| I/O 软件层分层模拟（用户层 → 设备无关 → 驱动 → 中断） | L0-L4 五层拓扑图 + SVG 连线 + IRP↓/IRQ↑ 标签 |
| read 全流程：请求转换 → 权限校验 → 指令下发 → 中断响应 → 结果返回 | 状态机逐步推进，每步附 stepDescription |
| 数据流转与缓冲模拟（单缓冲/双缓冲、用户态↔内核态数据传递） | 乒乓缓冲 A/B 切换 + 用户缓冲区面板 + 粒子动画 |
| 异常场景模拟（权限不足、非法地址、设备故障等） | 5 种故障注入：EACCES / EFAULT / EIO / EPERM / ENOENT |
| 单步执行清晰展示每步细节 | btn-step 单步 + 自动连点 + 系统调用日志 |
| 8 人小组协作 + 答辩展示 | 见下方分工 |
...
## I/O 软件层调用流程（对应课设四层 + 硬件）

```
L0 用户层 I/O 软件    sys_read(fd, buf, len)  — 发起 read 请求
      │ IRP ↓
L1 设备无关软件 (VFS)   路径解析 → 权限校验 → 缓冲分配 (乒乓 A/B)
      │ IRP ↓
L2 设备驱动程序          构造 I/O 请求包 → 下发硬件 → 进程阻塞
      │ IRP ↓
L4 硬件层 (磁盘控制器)   磁盘控制器执行 DMA 读取
      │ IRQ ↑
L3 中断处理程序 (ISR)    ISR 唤醒进程 → 数据拷贝到用户缓冲区 → 逐层返回
```

> 注：课设要求的四层软件层（用户层 I/O → 设备无关软件 → 设备驱动 → 中断处理）我们全部覆盖，并额外加入了硬件层使流程更完整。
...
| 前端可视化 (2人) | `web/src/` — SVG 五层拓扑图 + 连线动画、Canvas 粒子系统、手风琴状态面板、交互控制 |
| 前端样式与布局 (1人) | `web/src/style.css` + `index.html` — 7:3 宽屏排版、浅色实验室主题、响应式适配 |
